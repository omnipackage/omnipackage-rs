...
impl PackageInput {
    pub fn setup_rpm(&self) -> PackageOutput {
        let specfile_path_template_path = self.build_context.config.rpm.clone().unwrap().spec_template;

        let rpmbuild_folder_name = format!("{}-{}", self.build_context.config.package_name, self.build_context.distro.id);
        ...
        let source_folder_name = format!("{}-{}", self.build_context.config.package_name, self.build_context.job_variables.version);
        let specfile_name = format!("{}-{}.spec", source_folder_name, self.build_context.distro.id);

        let mut vars: HashMap<String, Var> = self.build_context.job_variables.to_vars();
        vars.extend(self.build_context.config.to_vars());
